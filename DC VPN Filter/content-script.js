...
async function ensureIpDb() {
  ...
  try {
    ...
    if (!data || typeof data !== "object" || Object.keys(data).length === 0) {
      console.error("[DC VPN Guest Filter] ip.json이 비어 있습니다. 필터를 비활성화합니다.");
      gIpDb = null;
      gIpDbReady = true;
      return false;
    }
    ...
    gIpDb = data;
    gIpDbReady = true;
    console.log("[DC VPN Guest Filter] ip.json 로드 완료. prefix 개수:", Object.keys(gIpDb).length);
    return true;
  } catch (e) {
    console.error("[DC VPN Guest Filter] ip.json 로드 실패. 필터를 비활성화합니다.", e);
    gIpDb = null;
    gIpDbReady = true; 
    return false;
  }
...
function classifyIp(ip) {
  if (!gIpDbReady || !gIpDb) {
    ...
    return { hasInfo: true, isVpn: false, info: null };
  }

  const info = getIpInfoFromDb(ip);
  if (!info) {
    return { hasInfo: false, isVpn: false, info: null }; // prefix 자체가 없음 = 정보 없음
  }
  const raw = String(info).trim();


  if (VPN_EXCLUDE_LABELS.some((label) => raw.includes(label))) {
    return { hasInfo: true, isVpn: false, info: raw };
  }
  ...
  return { hasInfo: true, isVpn: containsVpn, info: raw };
}
...
function isGuestWriter(el) {
  ...
  const uid = el.dataset.uid || el.getAttribute("data-uid") || "";
  const ip = el.dataset.ip || el.getAttribute("data-ip") || "";
  return !!ip && !uid;
}
...
function hideByWriterElement(writerEl) {
  if (!writerEl || writerEl.dataset.dcvpnHidden === "1") return;
...
async function processWriterElement(writerEl) {
  ...
  if (writerEl.dataset.dcvpnProcessed === "1") return;
  ...
  const ip = writerEl.dataset.ip || writerEl.getAttribute("data-ip");
  if (!ip) return;
  ...
  const { hasInfo, isVpn } = classifyIp(ip);
...
function scanExistingWriters() {
  document
    ...
    .forEach((el) => processWriterElement(el));
}

function startObserver() {
  if (gObserver || !document.body) return;
    ...
    for (const m of mutations) {
      for (const node of m.addedNodes) {
        ...
        if (node.matches(".ub-writer")) {
          ...
          const writers = node.querySelectorAll?.(".ub-writer");
          if (writers && writers.length) {
            writers.forEach((el) => processWriterElement(el));
          }
        }
      }
    }
